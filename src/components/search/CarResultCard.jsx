...
const CarResultCard = ({ car, onBook, vertical = false }) => {
  ...
  // ─── VERTICAL CARD (Search Results grid) ─────────────────────
  if (vertical) {
    return (
      <div 
        onClick={handleViewDetails}
        className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-xl hover:shadow-gray-200/50 transition-all duration-500 overflow-hidden group flex flex-col h-full cursor-pointer"
      >
        {/* Car Image */}
        <div className="relative h-48 bg-gray-100 overflow-hidden shrink-0">
          ...
          {rating && (
            <div className="absolute top-3 right-3 bg-gray-900/80 backdrop-blur-sm text-white px-2 py-1 rounded-lg text-xs font-bold flex items-center gap-1">
        ...
        <div className="p-5 flex flex-col flex-grow">
          ...
          <div className="flex items-center gap-3 mb-4 pb-4 border-b border-gray-100">
            <div className="w-9 h-9 rounded-full bg-gradient-to-br from-green-100 to-emerald-50 overflow-hidden flex items-center justify-center border-2 border-green-200">
          ...
          <div className="mt-auto pt-3 border-t border-gray-100 flex items-center justify-between gap-3">
            <div>
              ...
              <div className="text-xl font-extrabold text-gray-900">{formatCurrency(price)}</div>
            </div>
            <Button 
              onClick={(e) => { e.stopPropagation(); onBook && onBook(car); }}
              className="bg-green-600 hover:bg-green-700 text-white font-bold px-5 rounded-xl shadow-lg shadow-green-600/20"
            >
              Book Now
            </Button>
          </div>
  ...
  // ─── HORIZONTAL CARD (Transfer page list) ────────────────────
  return (
    <div 
        onClick={handleViewDetails}
